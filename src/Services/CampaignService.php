...
class CampaignService extends BaseService
{
    ...
    {
        // 1. Collect query builder for subscribers from all segments
        $subscribersQuery = Subscriber::query()
            ->whereHas('segments', function ($q) use ($campaign) {
                $q->whereIn('email_segments.id', $campaign->segments->pluck('id'));
            })
            ->where('status', 'subscribed')
            ->select('email_subscribers.*')
            ->distinct();

        // 2. Count total subscribers first
        ...
        // 3. Lấy batch size từ config
        $batchSize = config('email-marketing.batch_size', 100);

        // 4. Calculate total chunks
        $totalChunks = (int) ceil($totalSubscribers / $batchSize);

        // 5. Update campaign status before processing
        $campaign->update(['status' => 'sending']);

        $chunkIndex = 0;

        // 6. Chunk by ID - mỗi chunk có transaction riêng
        $subscribersQuery->chunkById($batchSize, function ($subscribers) use ($campaign, &$chunkIndex, $totalChunks) {
            $chunkIndex++;
